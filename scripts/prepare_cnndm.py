...
def prepare(
    destination_path: Path = Path("data/cnn-dm"),
...
def prepare_line(inputs: str, targets: str, tokenizer: Tokenizer, max_length: int):
    ...
    input_ids_no_response = torch.cat((prompt_ids, appendix_ids), dim=0)
    input_ids = torch.cat((input_ids_no_response, labels), dim=0)
    return {
        "input_ids": input_ids,
